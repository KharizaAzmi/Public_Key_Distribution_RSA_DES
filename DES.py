class des():
    ...
    def decimalToBinary(self, decimal_number):
        # Fungsi untuk mengonversi bilangan desimal menjadi representasi biner
        return bin(decimal_number)[2:]

    # def string_to_binary(input_string):
    #     # Mengonversi string ke representasi biner
    #     byte_representation = input_string.encode('utf-8')
    #     hex_representation = binascii.hexlify(byte_representation)
    ...
    def encryptDES(self, message, key):
        key_binary = self.decimalToBinary(key)
        # key = self.padding(key)
        # binary_key = self.stringToBinary(key)
        permuted_key = self.convertToPermutedKey(key_binary)
